...
function Sidebar({ section, page, goTo, setPage,
  ...
  // PageNav 요소들 (번역키 사용)
  const gradeNav = [
    { id: 'dashboard' as Page, labelKey: 'sideDashboard', Icon: LayoutGrid  },
    { id: 'credits'   as Page, labelKey: 'sideCredits',   Icon: Activity    },
    { id: 'timetable' as Page, labelKey: 'sideTimetable', Icon: Calendar    },
    { id: 'ai'        as Page, labelKey: 'sideAI',        Icon: HelpCircle  },
  ];
  ...
  return (
    <aside
      ...
      {gradeNav.map(({ id, labelKey, Icon }) => (
        <button
          ...
          className={`flex items-center gap-2 w-full rounded-[7px]
                      ...
                      justify-center mb-1 py-1.5 cursor-pointer
          ...
          <span className={inTxt}>{t(labelKey)}</span>
        </button>
      ))}
      ...
      <button
        ...
        <span className={inTxt}>{t('sideBoard')}</span>
      </button>
      ...
      <p className={`px-2.5 pb-1 text-[9px] font-bold text-(--text-3)
                     uppercase tracking-widest ${txt}`}>
        {t('sideCoursesSection')}
      </p>
      <button
        onClick={() => goTo('courses')}
        title={t('sideCoursesBtn')}
        className={`flex items-center gap-2 w-full rounded-[7px]
                    text-[12px] font-medium transition-colors
                    justify-center px-0 py-2.5 cursor-pointer
                    ${!collapsed ? 'lg:justify-start lg:px-2.5 lg:py-2' : ''}
        ...
        <span className={inTxt}>{t('sideCoursesBtn')}</span>
      </button>
      ...
      <div className={txt}>
        ...
        <div className="px-2.5 flex flex-col gap-1 mb-1">
          ...
          <div className="flex justify-between text-[11px]">
            <span className="text-(--text-3)">{t('sideThisSem')}</span>
            <b className="text-(--text-1)">
          ...
          <p className="text-[9px] text-(--text-3)">
            {t('sideGradCredits')} {gradesSummary ? `${gradesSummary.earnedTotal} / ${gradesSummary.gradTotal || 140} (${gradesSummary.gradPct}%)` : '- / -'}
          </p>
        </div>
        ...
        <div className="flex flex-col gap-0.5 mb-2">
          ...
          {events.length === 0 && (
            <p className="text-[10px] text-(--text-3) px-2.5 py-1">{t('sideNoEvents')}</p>
          )}
        </div>
      </div>
